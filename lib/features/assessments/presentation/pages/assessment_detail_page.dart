...
class AssessmentDetailPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    if (kDebugMode) {
      ...
      print('--- DEBUG BUILD: Assessment Detail Page ---');
              ...
              assessmentAsync.when(
                data: (assessment) {
                  ...
                  if (kDebugMode) {
                    ...
                    print('DEBUG DETAIL CARD: User ID: ${user?.id}, Access Count: ${accesses.length}, Assessment ID: ${assessment.id}, Computed UI State: ${access != null ? access.status : (isInCart ? 'in_cart' : 'add_to_cart')}');
                  }

                  final width = MediaQuery.of(context).size.width;
                  final isNarrow = width < 800;

                  final leftCardWidget = Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      AppCard(
                  ...
                  if (isNarrow) {
                    ...
                    return Row(
                      ...
                      children: [
                        Expanded(
                          flex: 2,
                          child: leftCardWidget,
                        ),
                        const SizedBox(width: AppSpacing.lg),
                        Expanded(
                          flex: 1,
                          child: rightCardWidget,
                        ),
                      ],
                    );
                  }
